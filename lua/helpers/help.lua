...
---@param bufnr integer
local function ts_query_get_ranges(parser, root, query_string, bufnr)
    local line_range = {}
...
---@param root TSNode
---@param bufnr integer
local function get_header_ranges(parser, root, bufnr)
    local header_ranges = ts_query_get_ranges(parser, root, string.format("(line . [(h1) (h2) (h3)]) @v", type), bufnr)
        ...
        if filetype == "help" or filetype == "markdown" then
            ...
            require("helpers.float").open({
                ...
                callback = function(buf)
                    ...
                    local code_ranges = ts_query_get_ranges(parser, root, "(code) @v", buf)
                    local header_ranges = get_header_ranges(parser, root, buf)
